...
import {
  Rifa,
  RifaNumero,
  RifaParticipanteConNumeros,
} from "@/lib/rifas/types";
...
import BotonVerRifa from "@/components/rifas/BotonVerRifa";

// 🔴 Importación de los componentes solicitados
import BotonCompartir from "@/components/rifas/BotonCompartir";
import BotonCerrarSesion from "@/components/rifas/BotonCerrarSesion";

interface PageProps {
  params: Promise<{ id: string }>;
}

export default function AdminRifaPage({ params }: PageProps) {
  const resolvedParams = use(params);
  const id = resolvedParams.id;

  const [rifa, setRifa] = useState<Rifa | null>(null);
  const [numerosMap, setNumerosMap] = useState<Map<number, RifaNumero>>(
    new Map(),
  );
  ...
  const [selectedParticipante, setSelectedParticipante] =
    useState<RifaParticipanteConNumeros | null>(null);

  // Estados de edición de la rifa
  ...
  const cargarDatos = useCallback(
    async (isInitialLoad = false) => {
      ...
      try {
        const { data: dataRifa, error } = await supabase
          .from("rifas")
          .select("*")
          .eq("id", id)
          .single();
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      alert(
        "Error al subir la imagen. Verifica la configuración de Supabase Storage.",
      );
    } finally {
  ...
  const handleToggleEstadoPago = async (
    ...
    if (
      nuevoEstado === "pagado" &&
      !window.confirm(
        "¿Estás seguro de marcar como PAGADO? Esto confirmará los números permanentemente.",
      )
    ) {
      return;
    }
    ...
    try {
      await actualizarEstadoPagoParticipante(
        participanteId,
        nuevoEstado,
        rifa.id,
      );
      ...
      if (nuevoEstado === "pagado") {
        ...
        alert("⚠️ Estado actualizado a pendiente.");
      }
    ...
    return (
      <div className="max-w-4xl mx-auto my-12 p-8 text-center bg-white border border-gray-200 rounded-3xl shadow-sm">
        ...
        <p className="text-gray-500 font-medium">
          No se encontró ninguna rifa asociada al identificador "{id}".
        </p>
      </div>
    );
  }

  const numerosEstadosMap = new Map<
    number,
    "ocupado" | "reservado" | "disponible"
  >();
  numerosMap.forEach((val, key) => {
    numerosEstadosMap.set(
      key,
      val.estado as "ocupado" | "reservado" | "disponible",
    );
  });

  const cantidadOcupados = Array.from(numerosEstadosMap.values()).filter(
    (e) => e === "ocupado",
  ).length;
  const cantidadReservados = Array.from(numerosEstadosMap.values()).filter(
    (e) => e === "reservado",
  ).length;
  const cantidadDisponibles =
    rifa.cantidad_numeros - (cantidadOcupados + cantidadReservados);
  ...
  const handleNumeroClick = (num: number) => {
    ...
    if (regNumero && regNumero.participante_id) {
      ...
      if (part) {
        ...
        alert(
          `El número #${num} está reservado pero no se encontró la ficha del participante.`,
        );
      }
    } else {
      alert(`El número #${num} se encuentra disponible.`);
    }
  };
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 space-y-8 bg-gray-50 min-h-screen text-gray-900">
      ...
      <div className="flex flex-wrap justify-between items-center gap-3 bg-white border border-gray-200 p-4 rounded-3xl shadow-sm">
        <div className="flex items-center gap-2">
          <BotonVerRifa slug={(rifa as any).slug || rifa.id} />
          <BotonCompartir slug={(rifa as any).slug || rifa.id} />
        </div>
        <BotonCerrarSesion />
      </div>
        ...
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="text-xs font-semibold text-green-700 bg-green-50 border border-green-200 hover:bg-green-100 px-3 py-1 rounded-full transition-all cursor-pointer"
            >
          ...
          <p className="text-sm text-gray-500">
            Premio: <strong className="text-gray-800">{rifa.premio}</strong>
          </p>
        </div>
      ...
      {editing && (
        <form
          ...
          className="bg-white border border-green-200 rounded-3xl p-6 space-y-6 shadow-lg shadow-green-600/5"
        >
          ...
          <div className="flex justify-end gap-3 pt-2">
            <button
              ...
              className="px-5 py-2.5 rounded-xl border border-gray-200 text-gray-600 font-bold text-sm hover:bg-gray-100 transition-all cursor-pointer"
            >
              Cancelar
            </button>
            <button
              ...
              className="px-6 py-2.5 bg-green-600 hover:bg-green-500 text-white font-bold rounded-xl text-sm transition-all shadow-md shadow-green-600/20 disabled:bg-gray-300 cursor-pointer"
            >
      ...
      <div className="bg-white border border-gray-200 rounded-3xl p-6 space-y-4 shadow-sm">
        <h2 className="text-lg font-bold text-gray-900">Mapa de Números</h2>
        <NumeroGrid
      ...
      <div className="bg-white border border-gray-200 rounded-3xl p-6 space-y-4 shadow-sm overflow-hidden">
        ...
        {participantes.length === 0 ? (
          <p className="text-gray-500 text-sm p-4 bg-gray-50 rounded-xl border">
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-gray-50/80 transition ${
                        estadoPago === "pagado" ? "bg-green-50/50" : ""
                      }`}
                    >
                      ...
                      <td className="py-3.5 px-4 font-mono text-green-700 font-medium whitespace-nowrap">
                        {p.telefono}
                      </td>
                      <td className="py-3.5 px-4">
                        <div className="flex flex-wrap gap-1 max-w-[200px]">
                          {p.numeros.map((n) => (
                            ...
                            >
                              #
                              {String(n).padStart(
                                String(rifa.cantidad_numeros).length,
                                "0",
                              )}
                            </span>
                          ))}
                      ...
                      <td className="py-3.5 px-4">
                        <button
                          ...
                          className="text-xs font-semibold bg-white hover:bg-gray-100 text-gray-700 px-4 py-2 rounded-xl border border-gray-200 transition-colors shadow-sm whitespace-nowrap cursor-pointer"
                        >
                          Ver Detalle
                        </button>
                      </td>
