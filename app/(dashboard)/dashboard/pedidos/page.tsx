...
export default async function PedidosPage() {
  ...
  if (!user) {
    redirect("/auth?redirect=%2Fdashboard%2Fpedidos");
  }
  ...
  if (catalogoError) {
    throw new Error("No pudimos consultar tu tienda.");
  }

  if (!catalogo) {
    redirect("/onboarding?next=%2Fdashboard%2Fpedidos");
  }
    ...
    <div className="space-y-7 text-[var(--text-primary)]">
      ...
      {pedidos.length === 0 ? (
        <div className="rounded-2xl border border-[var(--border-card)] bg-[var(--bg-card)] px-6 py-14 text-center">
          ...
          <Link
            href="/dashboard"
            className="mt-6 inline-flex rounded-xl bg-[var(--color-primary)] px-5 py-3 text-sm font-bold text-white"
          >
            Volver al dashboard
          </Link>
        </div>
        ...
        <div className="space-y-5">
          {pedidos.map((pedido) => {
            ...
            return (
              <article
                key={pedido.id}
                className="overflow-hidden rounded-2xl border border-[var(--border-card)] bg-[var(--bg-card)]"
              >
                ...
                <div className="grid gap-6 p-5 lg:grid-cols-2">
                      ...
                      {pedido.pedido_items.map((item) => (
                        ...
                        >
                          ...
                          <span className="shrink-0 font-semibold">
                            {formatearImporte(item.subtotal, pedido.moneda)}
                          </span>
                        </li>
                      ))}
                    </ul>
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    {pedido.comprador_email && (
                      <p>Correo: {pedido.comprador_email}</p>
                    )}
