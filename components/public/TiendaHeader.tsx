...
interface TiendaHeaderProps {
  /*
   * Conservamos el nombre de la propiedad para mantener
   * compatibilidad con TiendaClient. Dentro del componente
   * utilizamos el nombre "tienda".
   */
  catalogo: Tienda;
...
export default function TiendaHeader({
  ...
  /*
   * Detectamos si la página se ha desplazado para
   * mejorar la separación visual del encabezado.
   */
  useEffect(() => {
  ...
  /*
   * Administramos el foco, el teclado y el scroll
   * cuando el panel móvil se encuentra abierto.
   */
  useEffect(() => {
    if (!menuAbierto) {
      return;
      ...
      >
        <div className="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-4 md:w-auto">
            ...
            >
              {tienda.logo ? (
                <div className="relative flex h-16 w-48 items-center sm:w-60">
                  <Image
                    src={tienda.logo}
                ...
                <span className="max-w-[190px] truncate whitespace-nowrap text-xl font-bold tracking-tight sm:max-w-[240px]">
                  {tienda.nombre}
                </span>
              )}
            </Link>
          ...
          <div className="flex items-center gap-6 lg:gap-8">
            {categoriasSeguras.length > 0 && (
              <nav
                ...
                className="hidden items-center gap-5 py-1 lg:flex lg:gap-8"
              >
                {categoriasSeguras
                  .slice(0, 4)
                  .map((categoria) => (
                    <a
                      ...
                      className="relative max-w-[180px] truncate whitespace-nowrap py-2 text-sm font-medium uppercase tracking-wider transition-opacity duration-200 after:absolute after:bottom-0 after:left-0 after:h-[2px] after:w-full after:origin-bottom-right after:scale-x-0 after:bg-current after:transition-transform after:duration-200 hover:opacity-80 hover:after:origin-bottom-left hover:after:scale-x-100"
                    >
                      {categoria.nombre}
                    </a>
                  ))}
            ...
            <button
              ...
              className="rounded-full p-2 outline-none transition-transform duration-200 hover:scale-105 hover:bg-white/10 focus-visible:ring-2 focus-visible:ring-current"
            >
              {buscadorAbierto ? (
                <X
                  size={22}
